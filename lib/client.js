window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region \0dsh-css:/Users/admin/Desktop/python project/deepseek-harness/packages/client/ui-a2a-assistant/src/client/AssistantSettingsSection.module.css.mjs
		const css$1 = ".UMn2Aq_page{box-sizing:border-box;width:100%;max-width:840px;margin:0 auto;padding:28px 32px 48px}.UMn2Aq_header{margin-bottom:24px}.UMn2Aq_header h2{letter-spacing:0;margin:0;font-size:22px;line-height:30px}.UMn2Aq_header p,.UMn2Aq_sectionHead p{color:var(--dsw-alias-label-secondary);margin:6px 0 0;font-size:13px;line-height:20px}.UMn2Aq_section{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);border-radius:8px;overflow:hidden}.UMn2Aq_sectionHead{border-bottom:1px solid var(--dsw-alias-border-l2);justify-content:space-between;align-items:flex-start;gap:20px;padding:20px 22px;display:flex}.UMn2Aq_sectionHead h3{letter-spacing:0;margin:0;font-size:16px;line-height:24px}.UMn2Aq_status{border:1px solid var(--dsw-alias-border-l2);min-height:28px;color:var(--dsw-alias-label-secondary);border-radius:5px;flex:none;align-items:center;gap:7px;padding:0 10px;font-size:12px;display:inline-flex}.UMn2Aq_form{flex-direction:column;gap:18px;padding:22px;display:flex}.UMn2Aq_grid{grid-template-columns:minmax(0,1fr) minmax(180px,.55fr);gap:16px;display:grid}.UMn2Aq_field{min-width:0;color:var(--dsw-alias-label-primary);flex-direction:column;gap:7px;font-size:13px;font-weight:500;display:flex}.UMn2Aq_field input,.UMn2Aq_field select,.UMn2Aq_field textarea{box-sizing:border-box;border:1px solid var(--dsw-alias-border-l3);width:100%;min-width:0;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-base);font:inherit;border-radius:6px;outline:none;font-weight:400}.UMn2Aq_field input,.UMn2Aq_field select{height:38px;padding:0 11px}.UMn2Aq_field textarea{resize:vertical;padding:10px 11px;line-height:20px}.UMn2Aq_field input:focus,.UMn2Aq_field select:focus,.UMn2Aq_field textarea:focus{border-color:var(--dsw-alias-brand-primary);box-shadow:0 0 0 2px color-mix(in srgb, var(--dsw-alias-brand-primary) 16%, transparent)}.UMn2Aq_field input:disabled,.UMn2Aq_field select:disabled,.UMn2Aq_field textarea:disabled{opacity:.55;cursor:not-allowed}.UMn2Aq_field small{color:var(--dsw-alias-label-secondary);font-size:11px;font-weight:400;line-height:18px}.UMn2Aq_actions{border-top:1px solid var(--dsw-alias-border-l2);justify-content:space-between;align-items:center;gap:16px;min-height:58px;padding:10px 16px 10px 22px;display:flex}.UMn2Aq_actions>div{align-items:center;gap:8px;display:flex}.UMn2Aq_success{color:var(--dsw-alias-state-success-primary);font-size:12px}.UMn2Aq_failure{color:var(--dsw-alias-state-error-primary);font-size:12px}@media (width<=700px){.UMn2Aq_page{padding:20px 16px 36px}.UMn2Aq_sectionHead{flex-direction:column}.UMn2Aq_grid{grid-template-columns:minmax(0,1fr)}.UMn2Aq_actions{flex-direction:column;align-items:flex-start}.UMn2Aq_actions>div{justify-content:flex-end;width:100%}}";
		...
		var AssistantSettingsSection_module_css_default = {
			"actions": "UMn2Aq_actions",
			"failure": "UMn2Aq_failure",
			"field": "UMn2Aq_field",
			"form": "UMn2Aq_form",
			"grid": "UMn2Aq_grid",
			"header": "UMn2Aq_header",
			"page": "UMn2Aq_page",
			"section": "UMn2Aq_section",
			"sectionHead": "UMn2Aq_sectionHead",
			"status": "UMn2Aq_status",
			"success": "UMn2Aq_success"
		};
		//#endregion
		...
		const EMPTY_DRAFT = {
			...
			minimaxFormat: "",
			aliyunNlsTokenEnv: "",
			aliyunAccessKeyIdEnv: "",
			aliyunAccessKeySecretEnv: "",
			aliyunTokenRegionId: "",
			aliyunTokenURL: "",
			aliyunNlsAppKey: "",
			aliyunAsrURL: "",
		...
		function AssistantSettingsSection(props) {
			...
			const aliyunAccessKeyIdRef = current?.aliyunAccessKeyIdEnv ?? "ALIYUN_AK_ID";
			const aliyunAccessKeySecretRef = current?.aliyunAccessKeySecretEnv ?? "ALIYUN_AK_SECRET";
			...
			const [aliyunAccessKeyConfigured, setAliyunAccessKeyConfigured] = (0, react.useState)(false);
			...
			const [aliyunAccessKeyIdWritable, setAliyunAccessKeyIdWritable] = (0, react.useState)(true);
			const [aliyunAccessKeySecretWritable, setAliyunAccessKeySecretWritable] = (0, react.useState)(true);
			...
			(0, react.useEffect)(() => {
				...
				setDraft({
					...
					aliyunAccessKeyIdEnv: current.aliyunAccessKeyIdEnv,
					aliyunAccessKeySecretEnv: current.aliyunAccessKeySecretEnv,
					aliyunTokenRegionId: current.aliyunTokenRegionId,
					aliyunTokenURL: current.aliyunTokenURL,
			...
			(0, react.useEffect)(() => {
				...
				api.credentials.describe({ refs: [
					...
					aliyunCredentialRef,
					aliyunAccessKeyIdRef,
					aliyunAccessKeySecretRef,
					...
					const aliyunAccessKeyIdView = response.result.value.credentials[aliyunAccessKeyIdRef];
					const aliyunAccessKeySecretView = response.result.value.credentials[aliyunAccessKeySecretRef];
					...
					setAliyunAccessKeyConfigured((aliyunAccessKeyIdView?.configured ?? false) && (aliyunAccessKeySecretView?.configured ?? false));
					setAliyunAccessKeyIdWritable(aliyunAccessKeyIdView?.writable ?? true);
					setAliyunAccessKeySecretWritable(aliyunAccessKeySecretView?.writable ?? true);
			...
			const save = async () => {
				...
				try {
					...
					const aliyunAccessKeyId = draft.aliyunAccessKeyId.trim();
					if (aliyunAccessKeyId !== "") {
						const response = await api.credentials.set({
							ref: draft.aliyunAccessKeyIdEnv.trim(),
							value: aliyunAccessKeyId
						});
						if (!response.result.ok) throw new Error(response.result.error.message);
					}
					const aliyunAccessKeySecret = draft.aliyunAccessKeySecret.trim();
					if (aliyunAccessKeySecret !== "") {
						const response = await api.credentials.set({
							ref: draft.aliyunAccessKeySecretEnv.trim(),
							value: aliyunAccessKeySecret
						});
						if (!response.result.ok) throw new Error(response.result.error.message);
					}
					if (aliyunAccessKeyId !== "" || aliyunAccessKeySecret !== "") setAliyunAccessKeyConfigured(true);
			...
			const removeKey = async (ref, kind) => {
				...
				try {
					...
					} else if (kind === "aliyunAccessKey") {
						setAliyunAccessKeyConfigured(false);
						setDraft((previous) => ({
							...previous,
							aliyunAccessKeyId: "",
							aliyunAccessKeySecret: ""
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", { children: t("settings.aliyun.title") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: t("settings.aliyun.description") })] }), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
								className: AssistantSettingsSection_module_css_default.status,
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.StateDot, { state: aliyunAccessKeyConfigured || aliyunConfigured ? "done" : "warning" }), aliyunAccessKeyConfigured || aliyunConfigured ? t("settings.key.configured") : t("settings.key.missing")]
							})]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: AssistantSettingsSection_module_css_default.form,
							children: [
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
										className: AssistantSettingsSection_module_css_default.field,
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("settings.aliyunAccessKeyId.label") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
											type: "password",
											autoComplete: "off",
											value: draft.aliyunAccessKeyId,
											placeholder: aliyunAccessKeyConfigured ? t("settings.key.placeholderConfigured") : t("settings.key.placeholder"),
											disabled: busy || !aliyunAccessKeyIdWritable,
											onChange: (event) => {
												edit("aliyunAccessKeyId", event.target.value);
											}
										})]
									}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
										className: AssistantSettingsSection_module_css_default.field,
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("settings.aliyunAccessKeySecret.label") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
											type: "password",
											autoComplete: "off",
											value: draft.aliyunAccessKeySecret,
											placeholder: aliyunAccessKeyConfigured ? t("settings.key.placeholderConfigured") : t("settings.key.placeholder"),
											disabled: busy || !aliyunAccessKeySecretWritable,
											onChange: (event) => {
												edit("aliyunAccessKeySecret", event.target.value);
											}
										})]
									})]
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
										className: AssistantSettingsSection_module_css_default.field,
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("settings.aliyunAccessKeyIdEnv.label") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
											type: "text",
											value: draft.aliyunAccessKeyIdEnv,
											disabled,
											onChange: (event) => {
												edit("aliyunAccessKeyIdEnv", event.target.value);
											}
										})]
									}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
										className: AssistantSettingsSection_module_css_default.field,
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("settings.aliyunAccessKeySecretEnv.label") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
											type: "text",
											value: draft.aliyunAccessKeySecretEnv,
											disabled,
											onChange: (event) => {
												edit("aliyunAccessKeySecretEnv", event.target.value);
											}
										})]
									})]
								}),
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
										className: AssistantSettingsSection_module_css_default.field,
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("settings.aliyunTokenRegionId.label") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
											type: "text",
											value: draft.aliyunTokenRegionId,
											disabled,
											onChange: (event) => {
												edit("aliyunTokenRegionId", event.target.value);
											}
										})]
									}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
										className: AssistantSettingsSection_module_css_default.field,
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: t("settings.aliyunTokenURL.label") }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
											type: "url",
											value: draft.aliyunTokenURL,
											disabled,
											onChange: (event) => {
												edit("aliyunTokenURL", event.target.value);
											}
										})]
									})]
								}),
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("footer", {
							...
							children: [notice === void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {}) : /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								...
								aliyunAccessKeyConfigured ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(_deepseek_ai_dsh_client_ui_primitives.Button, {
									variant: "ghost",
									disabled: busy || !aliyunAccessKeyIdWritable || !aliyunAccessKeySecretWritable,
									onClick: () => {
										Promise.all([removeKey(aliyunAccessKeyIdRef, "aliyunAccessKey"), removeKey(aliyunAccessKeySecretRef, "aliyunAccessKey")]);
									},
									children: t("settings.aliyunAccessKey.remove")
								}) : null,
		...
		//#endregion
		//#region \0dsh-css:/Users/admin/Desktop/python project/deepseek-harness/packages/client/ui-a2a-assistant/src/client/WechatAssistantWorkspace.module.css.mjs
		const css = ".VrybWq_entry{box-sizing:border-box;width:calc(100% + 4px);height:42px;color:var(--dsw-alias-label-primary);cursor:pointer;font:inherit;background:0 0;border:0;border-radius:12px;flex:none;align-items:center;gap:8px;margin:4px -2px;padding:0 10px 0 8px;font-size:14px;line-height:22px;display:flex;overflow:hidden}.VrybWq_entry:hover{background:var(--dsw-alias-interactive-bg-hover)}.VrybWq_entryActive{background:var(--dsw-alias-interactive-bg-active)}.VrybWq_entryRail{border-radius:50%;justify-content:center;gap:0;width:36px;height:36px;margin:8px 0 10px;padding:0}.VrybWq_entryAvatar{border-radius:50%;flex:none;overflow:hidden}.VrybWq_entryLabel{white-space:nowrap;overflow:hidden}.VrybWq_beta{border:1px solid var(--dsw-alias-border-l3);color:var(--dsw-alias-label-secondary);border-radius:3px;margin-left:auto;padding:0 4px;font-size:9px;line-height:16px}.VrybWq_workspace{z-index:1;min-width:0;min-height:0;color:var(--dsw-alias-label-primary);pointer-events:auto;background:#f4f5f7;grid-template-columns:280px minmax(0,1fr);display:grid;position:absolute;top:0;bottom:0;right:0}.VrybWq_conversationSidebar{border-right:1px solid var(--dsw-alias-border-l2);background:#fff;flex-direction:column;min-width:0;min-height:0;display:flex}.VrybWq_sidebarHeader{flex:none;padding:14px 16px 10px}.VrybWq_sidebarHeader strong{font-size:16px;display:block}.VrybWq_onlineDot,.VrybWq_offlineDot{background:#00b42a;border-radius:50%;flex:none;width:8px;height:8px;display:inline-block}.VrybWq_offlineDot{background:#f53f3f}.VrybWq_connectingDot{background:#ff9a2e;border-radius:50%;flex:none;width:8px;height:8px;display:inline-block}.VrybWq_groupLabel{color:#8a9099;padding:12px 16px 5px;font-size:11px;font-weight:600}.VrybWq_conversationRow,.VrybWq_conversationActive{min-height:58px;color:inherit;text-align:left;cursor:pointer;background:0 0;border:0;border-radius:8px;grid-template-columns:36px minmax(0,1fr) auto;align-items:center;gap:10px;margin:2px 8px;padding:9px 10px;display:grid}.VrybWq_conversationRow:hover,.VrybWq_conversationActive{background:#f1f2f4}.VrybWq_conversationCopy{min-width:0}.VrybWq_conversationCopy strong,.VrybWq_conversationCopy small{text-overflow:ellipsis;white-space:nowrap;display:block;overflow:hidden}.VrybWq_conversationCopy strong{font-size:14px;font-weight:500}.VrybWq_conversationCopy small{color:#8a9099;margin-top:3px;font-size:12px}.VrybWq_conversationRow time,.VrybWq_conversationActive time{color:#a0a5ad;align-self:start;padding-top:2px;font-size:10px}.VrybWq_pairEmpty{color:#a0a5ad;padding:9px 16px;font-size:12px}.VrybWq_chat{background:#f4f5f7;flex-direction:column;min-width:0;min-height:0;display:flex}.VrybWq_chatHeader{border-bottom:1px solid var(--dsw-alias-border-l2);background:#fff;flex:none;justify-content:space-between;align-items:center;gap:12px;min-height:58px;padding:0 16px;display:flex}.VrybWq_chatIdentity{align-items:center;gap:6px;min-width:0;display:flex}.VrybWq_chatIdentity strong,.VrybWq_chatIdentity span{text-overflow:ellipsis;white-space:nowrap;display:block;overflow:hidden}.VrybWq_chatIdentity strong{font-size:15px;font-weight:600}.VrybWq_chatIdentity span{color:#858b95;margin-top:3px;font-size:12px}.VrybWq_headerActions{flex:none;align-items:center;gap:8px;display:flex}.VrybWq_participants{padding-left:7px;display:flex}.VrybWq_participants span{border:2px solid #fff;border-radius:50%;margin-left:-7px;overflow:hidden}.VrybWq_call,.VrybWq_callActive,.VrybWq_more{border:1px solid var(--dsw-alias-border-l3);color:#4d5561;cursor:pointer;height:34px;font:inherit;background:#fff;border-radius:6px;justify-content:center;align-items:center;gap:6px;padding:0 10px;font-size:12px;display:flex}.VrybWq_call:hover,.VrybWq_more:hover{background:#f5f6f7}.VrybWq_callActive{color:#d9363e;border-color:#f53f3f}.VrybWq_more{width:34px;padding:0}.VrybWq_mobileBack{background:0 0;border:0;place-items:center;width:32px;height:32px;display:none}.VrybWq_callStrip{color:#315d9b;background:#eef4ff;border-bottom:1px solid #dfe6f3;flex:none;align-items:center;gap:7px;padding:8px 16px;font-size:12px;display:flex}.VrybWq_callStrip span{color:#71809a;margin-left:auto}.VrybWq_pauseListening,.VrybWq_resumeListening{color:#315d9b;cursor:pointer;height:28px;font:inherit;background:#fff;border:1px solid #b8c9e5;border-radius:5px;flex:none;align-items:center;gap:5px;padding:0 9px;font-size:11px;display:flex}.VrybWq_pauseListening:hover{background:#f7faff}.VrybWq_pauseListening b,.VrybWq_resumeListening b{font-weight:400}.VrybWq_resumeListening{color:#fff;background:#3370ff;border-color:#3370ff}.VrybWq_resumeListening:hover{background:#245bdb}.VrybWq_remoteAudio{opacity:0;pointer-events:none;width:1px;height:1px;position:absolute}.VrybWq_messages{flex:1;min-height:0;padding:18px 24px;overflow:hidden auto}.VrybWq_messageFlow{flex-direction:column;gap:10px;max-width:780px;min-height:100%;margin:0 auto;display:flex}.VrybWq_empty{color:#959ba4;flex:1;place-items:center;font-size:13px;display:grid}.VrybWq_messageMine,.VrybWq_messageAgent{align-items:flex-start;gap:8px;display:flex}.VrybWq_messageMine{flex-direction:row-reverse}.VrybWq_bubbleWrap{flex-direction:column;gap:4px;max-width:68%;display:flex}.VrybWq_bubbleWrap>span{color:#8a9099;margin:0 8px;font-size:11px}.VrybWq_messageMine .VrybWq_bubbleWrap>span{text-align:right}.VrybWq_bubbleWrap p{color:#202124;overflow-wrap:anywhere;white-space:pre-wrap;background:#fff;border-radius:4px 14px 14px;margin:0;padding:9px 13px;font-size:14px;line-height:1.55}.VrybWq_messageMine .VrybWq_bubbleWrap p{color:#fff;background:#3370ff;border-radius:14px 4px 14px 14px}.VrybWq_bubbleWrap .VrybWq_liveBubble{min-width:42px;min-height:22px}.VrybWq_voiceBars{justify-content:center;align-items:center;gap:3px;width:38px;height:22px;display:flex}.VrybWq_voiceBars i{background:#3370ff;border-radius:2px;width:3px;min-height:6px;max-height:22px;transition:height 80ms linear;display:block}.VrybWq_messageMine .VrybWq_voiceBars i{background:#fff}.VrybWq_bubbleWrap time{color:#a0a5ad;padding:0 4px;font-size:10px}.VrybWq_messageMine .VrybWq_bubbleWrap time{text-align:right}.VrybWq_systemMessage{color:#858b95;background:#0000000f;border-radius:12px;align-self:center;max-width:90%;padding:6px 12px;font-size:12px}.VrybWq_replying{color:#858b95;font-size:12px}.VrybWq_composer{border-top:1px solid var(--dsw-alias-border-l2);background:#fff;flex:none;padding:10px 16px}.VrybWq_inputRow{align-items:flex-end;gap:8px;max-width:820px;margin:0 auto;display:flex}.VrybWq_inputRow textarea{resize:vertical;box-sizing:border-box;color:#202124;min-height:40px;max-height:120px;font:inherit;background:#fff;border:1px solid #d8dbe0;border-radius:6px;flex:1;padding:10px 12px;font-size:14px;line-height:20px}.VrybWq_inputRow button{color:#fff;cursor:pointer;background:#3370ff;border:0;border-radius:6px;flex:none;place-items:center;width:40px;height:40px;display:grid}.VrybWq_inputRow button:disabled{opacity:.45;cursor:default}.VrybWq_sendHint{color:#9ba0a8;text-align:right;max-width:820px;margin:5px auto 0;font-size:10px;display:block}.VrybWq_error{color:#d9363e;max-width:820px;margin:0 auto 6px;font-size:12px}body[data-ds-dark-theme] .VrybWq_workspace,body[data-ds-dark-theme] .VrybWq_chat{background:#1b1d20}body[data-ds-dark-theme] .VrybWq_conversationSidebar,body[data-ds-dark-theme] .VrybWq_sidebarHeader,body[data-ds-dark-theme] .VrybWq_chatHeader,body[data-ds-dark-theme] .VrybWq_composer{background:#222428}body[data-ds-dark-theme] .VrybWq_conversationRow:hover,body[data-ds-dark-theme] .VrybWq_conversationActive{background:#303338}body[data-ds-dark-theme] .VrybWq_groupLabel,body[data-ds-dark-theme] .VrybWq_conversationCopy small,body[data-ds-dark-theme] .VrybWq_chatIdentity span{color:#a6acb5}body[data-ds-dark-theme] .VrybWq_participants span{border-color:#222428}body[data-ds-dark-theme] .VrybWq_call,body[data-ds-dark-theme] .VrybWq_more,body[data-ds-dark-theme] .VrybWq_bubbleWrap p,body[data-ds-dark-theme] .VrybWq_inputRow textarea{color:#eef0f3;background:#292c31}body[data-ds-dark-theme] .VrybWq_messageMine .VrybWq_bubbleWrap p{color:#fff;background:#3370ff}body[data-ds-dark-theme] .VrybWq_callStrip{color:#b9c9e5;background:#222b38;border-color:#343d4c}body[data-ds-dark-theme] .VrybWq_callStrip span{color:#98a8c1}body[data-ds-dark-theme] .VrybWq_pauseListening{color:#c3d0e5;background:#292f39;border-color:#4c5b72}@media (width<=760px){.VrybWq_workspace{grid-template-columns:minmax(0,1fr)}.VrybWq_conversationSidebar.VrybWq_mobileHidden,.VrybWq_chat{display:none}.VrybWq_chat.VrybWq_mobileChat{display:flex}.VrybWq_mobileBack{display:grid}.VrybWq_participants,.VrybWq_call span,.VrybWq_callActive span{display:none}.VrybWq_call,.VrybWq_callActive{width:34px;padding:0}.VrybWq_callStrip{white-space:nowrap}.VrybWq_callStrip span{display:none}.VrybWq_pauseListening,.VrybWq_resumeListening{justify-content:center;width:30px;padding:0}.VrybWq_pauseListening b,.VrybWq_resumeListening b{display:none}.VrybWq_messages{padding:14px 12px}.VrybWq_bubbleWrap{max-width:84%}}";
		...
		var WechatAssistantWorkspace_module_css_default = {
			"beta": "VrybWq_beta",
			"bubbleWrap": "VrybWq_bubbleWrap",
			"call": "VrybWq_call",
			"callActive": "VrybWq_callActive",
			"callStrip": "VrybWq_callStrip",
			"chat": "VrybWq_chat",
			"chatHeader": "VrybWq_chatHeader",
			"chatIdentity": "VrybWq_chatIdentity",
			"composer": "VrybWq_composer",
			"connectingDot": "VrybWq_connectingDot",
			"conversationActive": "VrybWq_conversationActive",
			"conversationCopy": "VrybWq_conversationCopy",
			"conversationRow": "VrybWq_conversationRow",
			"conversationSidebar": "VrybWq_conversationSidebar",
			"empty": "VrybWq_empty",
			"entry": "VrybWq_entry",
			"entryActive": "VrybWq_entryActive",
			"entryAvatar": "VrybWq_entryAvatar",
			"entryLabel": "VrybWq_entryLabel",
			"entryRail": "VrybWq_entryRail",
			"error": "VrybWq_error",
			"groupLabel": "VrybWq_groupLabel",
			"headerActions": "VrybWq_headerActions",
			"inputRow": "VrybWq_inputRow",
			"liveBubble": "VrybWq_liveBubble",
			"messageAgent": "VrybWq_messageAgent",
			"messageFlow": "VrybWq_messageFlow",
			"messageMine": "VrybWq_messageMine",
			"messages": "VrybWq_messages",
			"mobileBack": "VrybWq_mobileBack",
			"mobileChat": "VrybWq_mobileChat",
			"mobileHidden": "VrybWq_mobileHidden",
			"more": "VrybWq_more",
			"offlineDot": "VrybWq_offlineDot",
			"onlineDot": "VrybWq_onlineDot",
			"pairEmpty": "VrybWq_pairEmpty",
			"participants": "VrybWq_participants",
			"pauseListening": "VrybWq_pauseListening",
			"remoteAudio": "VrybWq_remoteAudio",
			"replying": "VrybWq_replying",
			"resumeListening": "VrybWq_resumeListening",
			"sendHint": "VrybWq_sendHint",
			"sidebarHeader": "VrybWq_sidebarHeader",
			"systemMessage": "VrybWq_systemMessage",
			"voiceBars": "VrybWq_voiceBars",
			"workspace": "VrybWq_workspace"
		};
		//#endregion
		...
		const SECRETARY_VOICE_THRESHOLD = .038;
		const SECRETARY_PREFIX_MS = 450;
		const SECRETARY_MAX_SEGMENT_MS = 8e3;
		...
		function readAsrError(body, status) {
			try {
				const parsed = JSON.parse(body);
				const message = typeof parsed.message === "string" ? parsed.message : typeof parsed.Message === "string" ? parsed.Message : typeof parsed.status_text === "string" ? parsed.status_text : typeof parsed.error_message === "string" ? parsed.error_message : void 0;
				const detail = [typeof parsed.Code === "string" ? parsed.Code : typeof parsed.code === "string" ? parsed.code : typeof parsed.status === "number" ? String(parsed.status) : void 0, message].filter(Boolean).join(": ");
		...
		function fallbackSessionId(snapshot) {
			...
			return snapshot.ids.map((id) => snapshot.byId[id]).filter((session) => session !== void 0 && !session.blank).sort((left, right) => right.updatedAt - left.updatedAt)[0]?.id;
		}
		...
		function hostMessagesToConversations(hostMessages) {
			...
			for (const message of hostMessages) next[message.conversation].push({
		...
		async function postHostMessage(conversation, role, text) {
			...
			if (!response.ok) throw new Error(`Assistant bridge returned ${response.status}`);
			const body = await response.json();
			if (body.message === void 0) throw new Error("Assistant bridge returned an invalid message");
			return body.message;
		}
		...
		function WechatAssistantWorkspace({ useSessions, workspace, settings, resolveSession, send, t }) {
			...
			const [boundSessionId, setBoundSessionId] = (0, react.useState)(void 0);
			const sessionId = resolveSession(boundSessionId) === void 0 ? fallbackId : boundSessionId;
			...
			const [messages, setMessages] = (0, react.useState)(EMPTY_MESSAGES);
			...
			const secretarySegmentStartedAt = (0, react.useRef)(void 0);
			...
			const voiceSilenceMs = (0, react.useRef)(settingsSnapshot.value?.voiceSilenceMs ?? 1500);
			...
			(0, react.useEffect)(() => {
				if (!open || boundSessionId !== void 0 || sessionId === void 0) return;
				postAssistantState(sessionId).then((state) => {
			...
			(0, react.useEffect)(() => {
				...
				busy.current = false;
				setSending(false);
				if (hostReplyId !== null) postHostReply(hostReplyId, text).then(fetchHostMessages).then((hostMessages) => {
			...
			(0, react.useEffect)(() => {
				...
				const sync = async () => {
					...
					if (disposed) return;
					if (state.secretarySessionId !== void 0 && state.secretarySessionId !== boundSessionId) setBoundSessionId(state.secretarySessionId);
					setMessages(hostMessagesToConversations(hostMessages));
					if (sessionId === void 0 || busy.current || snapshot?.running === true) return;
					const pending = hostMessages.find((message) => message.conversation === "self" && message.role === "user" && message.status === "pending" && !processingHostMessageIds.current.has(message.id));
			...
			function resetSecretarySegment() {
				...
				secretarySegmentStartedAt.current = void 0;
			}
			...
			async function submit(raw) {
				...
				if (conversation === "chatgpt") {
					...
					postHostMessage("chatgpt", "user", text).catch(() => {});
					try {
						call.sendText(text);
					} catch (failure) {
						setError(failure instanceof Error ? failure.message : String(failure));
			...
			function handleSecretaryAudio(input, inputSampleRate) {
				...
				if (level >= SECRETARY_VOICE_THRESHOLD) {
					...
					secretarySilentSince.current = void 0;
					if (secretarySegmentStartedAt.current !== void 0 && now - secretarySegmentStartedAt.current >= SECRETARY_MAX_SEGMENT_MS) finishSecretarySegment();
					return;
				}
				if (!secretaryRecording.current) {
			...
			const startRealtimeCall = async () => {
				...
				const call = new OpenAIRealtimeCall({
					...
					onUserTranscript: (text) => {
						...
						postHostMessage("chatgpt", "user", transcript).catch(() => {});
					},
					...
					onAssistantTranscript: (text) => {
						...
						postHostMessage("chatgpt", "assistant", transcript).catch(() => {});
					},
		...
		const zh = {
			...
			"status.noSessionError": "没有可用的 Harness Session，请先在本机创建一个会话",
			"status.sending": "正在回复",
			"status.failed": "发送失败",
			"settings.nav": "微信助手",
			"settings.title": "微信助手",
			...
			"settings.voiceSilenceMs.hint": "同一套等待时间用于 ChatGPT Realtime 和秘书语音电话；走路环境建议 1500 毫秒。",
		...
		const en = {
			...
			"status.noSessionError": "No Harness Session is available. Create one on this device first.",
			...
			"settings.voiceSilenceMs.hint": "The same wait time is shared by ChatGPT Realtime and Secretary voice calls; 1500 ms is recommended while walking.",
