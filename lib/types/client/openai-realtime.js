...
export class OpenAIRealtimeCall {
        ...
        try {
            ...
            peer.onconnectionstatechange = () => {
                if (peer.connectionState === 'failed')
                    this.callbacks.onError('OpenAI Realtime connection failed');
            };
            const offer = await peer.createOffer();
            await peer.setLocalDescription(offer);
            const sdp = offer.sdp;
            if (sdp === undefined || sdp.trim() === '')
            ...
            if (!tokenResponse.ok)
                throw new Error(readError(tokenBody, tokenResponse.status));
            ...
            if (!response.ok)
                throw new Error(readError(answer, response.status));
    ...
    sendText(text) {
        ...
        if (channel?.readyState !== 'open')
            throw new Error('Start the ChatGPT Live call first');
    ...
    stop() {
        ...
        for (const track of this.input?.getTracks() ?? [])
            track.stop();
        this.input = undefined;
        this.peer?.close();
        this.peer = undefined;
        if (this.meterFrame !== undefined)
    ...
    createMeter(stream) {
        ...
        if (context === undefined)
            throw new Error('Audio meter is not initialized');
    ...
    scheduleVoiceResponse() {
        this.clearResponseTimer();
        if (this.muted || this.responseInFlight)
            return;
        this.responseTimer = setTimeout(() => {
            this.responseTimer = undefined;
            const channel = this.channel;
            if (channel?.readyState !== 'open' || this.muted || this.responseInFlight)
                return;
            this.responseInFlight = true;
            channel.send(JSON.stringify({ type: 'response.create', response: { output_modalities: ['audio'] } }));
